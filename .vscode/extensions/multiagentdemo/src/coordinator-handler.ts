...
export const getHandler = (roleDescription:string, toolsTag:string = 'multi-agent'): vscode.ChatRequestHandler => {
    return async (
    ...
    if (request.command === 'list') {
        stream.markdown(`Available tools: ${vscode.lm.tools.filter(tool => tool.tags.includes(toolsTag)).join(', ')}\n\n`);
        return;
    }
    ...
    const tools = vscode.lm.tools.filter(tool => tool.tags.includes(toolsTag));
